...
::: {.incremental}
- $Ruv$ is sometimes written $uRv$ and read: "$v$ is **accessible from** $u$." 

    To be accessible from $u$ is to be possible relative to $u$.

-  $w \in V(p)$ is sometimes read: "$p$ is **true at** $w$."

    $V(p)$ is the set of worlds at which $p$ is true.
:::

## graphs for possible worlds models {.smaller}

We will use the directed graph to depict a possible worlds model $(W, R, V)$ where:
...
![](4d1.png){width="400px" fig-align="center"}
:::
:::
...
\end{array}
$$
Note that $\Box \varphi$ is true at $w$ iff $\varphi$ is true at **every accessible world** from $u$.
...
Note that $\Diamond \varphi$ is true at $w$ iff $\varphi$ is true at **some accessible world** from $u$.
:::
...
- $p \to (p \to q)$
    ...
    True at $w_1$, $w_2$ and $w_3$. Why?

- $\Box p$
    
    True at $w_3$ and $w_4$. Why?
...
1. $\Diamond (p \vee q) \leftrightarrow (\Diamond p \vee \Diamond q)$
    ...
    $p$ or $q$ *can* be the case iff $p$ *can* be the case or $q$ *can* be the case.<br/><br/>

2. $\Box (p \wedge q) \leftrightarrow (\Box p \wedge \Box q)$

    $p$ and $q$ *must* be the case iff $p$ *must* be the case or $q$ *must* be the case.
:::
::: {.column width="50%"}
...
3. $\Diamond (p \wedge q) \leftrightarrow (\Diamond p \wedge \Diamond q)$
    ...
    For some $p$ and $q$, $p$ and $q$ *can* each be the case even if $p$ and $q$ *cannot* jointly be the case.

4. $\Box (p \vee q) \leftrightarrow (\Box p \vee \Box q)$

   For some $p$ and $q$, $p$ or $q$ *must* be the case even if none of them *must* be the case.
:::
:::
:::
