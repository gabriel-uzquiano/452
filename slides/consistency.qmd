...
($\Rightarrow$) Assume $\Gamma \vdash \varphi$. Suppose, for a reductio, that $\varphi \notin \Gamma$. By definition of *maximal consistency*, $\Gamma, \varphi$ is inconsistent. Since $\Gamma \vdash \varphi$, $\Gamma \vdash \bot$ if $\Gamma, \varphi \vdash \bot$. So, $\Gamma$ is inconsistent. Contradiction.
<br/>
<br/>
...
The justification for $\Gamma \nvdash \varphi$ is as follows. Since $\vdash \neg \varphi \to (\varphi \to \bot)$ (4.7), we have that $\Gamma \vdash \varphi$ only if $\Gamma \vdash \bot$ by \textsf{MP}.
<br/>
<br/>
...
2. from $\Sigma$ is *maximal consistent* to $\Sigma$ is *satisfiable*.
...
3. from $\Sigma$ is *satisfiable* to $\Gamma$ is *satisfiable*.
